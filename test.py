...
def database_to_dict2(table_name, connection) :
    df_res = pd.read_sql_query('SELECT * FROM '+table_name, connection) #on récupère DF de la BDD pour avoir le bon id
    list_res=df_res.to_dict(orient='records') # crée une liste de dico en clé  -> valeur 1ere col, en valeur -> valeur 2eme col
...
###  TEST  ###
filepath= "./donnees/fiches_mensuelles/Aout-2023.csv"

df=pd.read_csv(filepath)
df=select_activite(df)
...
print(before_dico_type_activite_db)

df_type_activite=df[['type_activite_nom','activite_nom']].drop_duplicates()
print(df_type_activite)

# df_typ_act = pd.read_sql_query('SELECT * FROM type_activite', conn)
# print(df_typ_act)

df = pd.DataFrame(np.arange(9).reshape(3, 3),
                  columns=['type_activite', 'type_activite_nom','activite_nom'])
print(df)
list_res=df.to_dict(orient='records') 
print(list_res)
...
def database_to_dict2(table_name, connection) :
    ...
    dict_res={}  #on tranforme la liste de dico en dico valeur(k), id(v)
    for i in list_res:
        (dict_res[i[table_name+'_nom']],dict_res[i['activite_nom']])=i[table_name+'_id']
    return dict_res