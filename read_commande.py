...
def select_commmande(df) :
    ...
    df_commande = df_commande.rename(columns={'Structure': 'type_structure_nom', 'Transaction': 'type_transaction_nom', 'Moyen de paiement':'moyen_paiement_nom', "Nom":'client_id',"Date d'achat":'commande_date_achat'}) #on change nom col pour correspondre au nom dans les tables type_structure, etc
    return df_commande
...
def excel_to_sql_date(date):
    patt1=re.compile(r"/")
    date=re.sub(patt1,"-",date) #on transforme les "/" en "-"
    patt2=re.compile(r"(\d\d)-(\d\d)-(\d{4})")
    date=re.sub(patt2,r"\3-\2-\1",date) #on inverse les jours et les mois
    return date
...
for filepath in filepaths :
    ...
    df_commande=select_commmande(df)
    ...
    df_commande= df_commande.replace(after_dico_type_structure_db) #on transforme noms de vendeur en leur id
    df_commande= df_commande.replace(after_dico_type_transaction_db) #on transforme les noms de type_activite en leur id
    df_commande= df_commande.replace(after_dico_moyen_paiement_db) #on transforme les noms de type_activite en leur id
